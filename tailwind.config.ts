  ...
  plugins: [
    ...
    plugin(function ({ addUtilities, addComponents }) {
      addUtilities({
        ...
        '.flipped-front': {
          transform:
            'rotateY(180deg) rotateX(-15deg) translatey(calc(-1 * var(--card-height) / 10))'
        },
        '.flipped-non-front': {
      ...
      addComponents({
        '.card': {
          ...
          transition:
            'transform 2s ease-in-out, top 1s ease-in-out, opacity 150ms ease-in-out',
          'backface-visibility': 'hidden'
        },
        '.button-fill-base': {
          'background-size': '0%',
        ...
        '.acceptance-card-visible': {
          ...
          transform:
            'rotateX(4deg) translateY(calc(var(--available-height)*0.15))'
        },

        '.accepted-card': {
          transform:
            'translateZ(calc(var(--letter-size) * 8)) rotateX(10deg) translateY(100svh)'
        },
        '.accepted-card-visible': {
          transform:
            'translateZ(10px) rotateX(0deg) translateY(calc(var(--card-height) * 0.01))'
        }
      })
    })
  ],
